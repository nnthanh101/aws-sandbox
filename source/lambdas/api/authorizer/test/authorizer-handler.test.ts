...
vi.mock("sandbox-authorizer/authorization.js");

let testEnv = generateSchemaData(AuthorizerLambdaEnvironmentSchema);
let handler: typeof import("sandbox-authorizer/authorizer-handler.js").handler;
...
beforeAll(async () => {
  ...
  handler = (
    await import("sandbox-authorizer/authorizer-handler.js")
  ).handler;
});
...
describe("handler", async () => {
  ...
  it("returns authorized response when authorization succeeds", async () => {
    const authorization = await import(
      "sandbox-authorizer/authorization.js"
    );
