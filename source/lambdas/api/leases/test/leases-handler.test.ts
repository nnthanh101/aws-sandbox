...
import { base64EncodeCompositeKey } from "sandbox-commons/data/encoding.js";
import {
  GlobalConfig,
  GlobalConfigSchema,
} from "sandbox-commons/data/global-config/global-config.js";
import { DynamoLeaseTemplateStore } from "sandbox-commons/data/lease-template/dynamo-lease-template-store.js";
import {
  BudgetConfigSchema,
...
import { LeaseLambdaEnvironmentSchema } from "sandbox-commons/lambda/environments/lease-lambda-environment.js";
...
let handler: typeof import("sandbox-leases/leases-handler.js").handler;

beforeAll(async () => {
  bulkStubEnv(testEnv);
  handler = (await import("sandbox-leases/leases-handler.js"))
    .handler;
});
...
describe("Leases Handler", async () => {
  ...
  describe("POST /leases", () => {
    ...
    it("should return 409 for when no accounts are available to lease", async () => {
      ...
      vi.spyOn(Sandbox, "approveLease").mockImplementation(() => {
    ...
    it("should return 409 when org api throws AccountNotFoundException", async () => {
      ...
      vi.spyOn(Sandbox, "approveLease").mockRejectedValue(
    ...
    it("should return 201 for auto approval lease request with valid inputs", async () => {
      ...
      vi.spyOn(Sandbox, "approveLease").mockReturnValue(
        Promise.resolve({
    ...
    describe("Lease Assignment Flow", () => {
      ...
      it("should return 403 when user role tries to create lease for another user", async () => {
        ...
        // Spy on requestLease
        const requestLeaseSpy = vi.spyOn(Sandbox, "requestLease");
