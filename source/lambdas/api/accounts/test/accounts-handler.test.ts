...
import { AccountLambdaEnvironmentSchema } from "sandbox-commons/lambda/environments/account-lambda-environment.js";
...
let handler: typeof import("sandbox-accounts/accounts-handler.js").handler;

beforeAll(async () => {
  handler = (
    await import("sandbox-accounts/accounts-handler.js")
  ).handler;
...
describe("Accounts Handler", () => {
  ...
  describe("POST /accounts", () => {
    ...
    it("should return 201 with valid input", async () => {
      ...
      vi.spyOn(Sandbox, "registerAccount").mockResolvedValue(account);
      expect(await handler(event, mockAuthorizedContext(testEnv))).toEqual({
        statusCode: 201,
        body: JSON.stringify({
          status: "success",
    ...
    it("should return 409 when org api throws AccountNotFoundException", async () => {
      ...
      vi.spyOn(Sandbox, "registerAccount").mockRejectedValue(
  ...
  describe("POST /accounts/{awsAccountId}/eject", () => {
    ...
    it("should return 409 when org api throws AccountNotFoundException", async () => {
      ...
      vi.spyOn(Sandbox, "ejectAccount").mockRejectedValue(
        new AccountNotFoundException({
          message: "mock exception",
          $metadata: {},
        }),
  ...
  describe("POST /accounts/{awsAccountId}/retryCleanup", () => {
    ...
    it("should return 409 when org api throws AccountNotFoundException", async () => {
      ...
      vi.spyOn(Sandbox, "retryCleanup").mockRejectedValue(
        new AccountNotFoundException({
          message: "mock exception",
          $metadata: {},
        }),
