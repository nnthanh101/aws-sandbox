...
export function overrideRetentionPolicies(
...
export const yesNoValues: (keyof typeof YesNoType)[] = [
  YesNoType.Yes,
  YesNoType.No,
];

export function yesNoCondition(
  scope: Construct,
  id: string,
  value: string,
): CfnCondition {
  return new CfnCondition(scope, id, {
    expression: Fn.conditionEquals(value, YesNoType.Yes),
  });
}
...
export function enabledDisabledCondition(
  scope: Construct,
  id: string,
  value: string,
): CfnCondition {
  return new CfnCondition(scope, id, {
    expression: Fn.conditionEquals(value, EnabledDisabledType.Enabled),
  });
}
...
export function addParameterLabel(
  parameter: CfnParameter,
  label: string,
): void {
  const stack = Stack.of(parameter);
  initCfnInterface(stack);
  stack.templateOptions.metadata![cfnInterfaceKey][parameterLabelsKey][
    parameter.logicalId
  ] = { default: label };
}
...
export function addParameterGroup(
  scope: Construct,
  ...
  stack.templateOptions.metadata![cfnInterfaceKey][parameterGroupsKey].push({
    ...(group.label && { Label: { default: group.label } }),
    Parameters: group.parameters.map((parameter) => parameter.logicalId),
  });
}
...
export class EnabledDisabledParameter extends ParameterWithLabel {
  ...
  getCondition(): CfnCondition {
    if (!this.condition) {
      this.condition = enabledDisabledCondition(
        this.stack,
        this.conditionId,
        this.valueAsString,
      );
