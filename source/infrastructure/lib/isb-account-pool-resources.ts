...
import { sharedAccountPoolSsmParamName } from "sandbox-commons/types/isb-types.js";
...
import {
  ...
  getSandboxWriteProtectionScp,
} from "sandbox-infrastructure/components/service-control-policies/isb-get-scp";
import {
  getContextFromMapping,
  getSolutionContext,
} from "sandbox-infrastructure/helpers/cdk-context";
...
import { IsbSandboxAccountStack } from "sandbox-infrastructure/isb-sandbox-account-stack";
import { AccountPoolConfig } from "sandbox-shared-json-param-parser/src/shared-json-param-parser-handler.js";
...
export class IsbAccountPoolResources {
  constructor(scope: Construct, props: IsbAccountPoolResourcesProps) {
    ...
    const sandboxOu = new CfnOrganizationalUnit(
      scope,
      "SandboxAccountPoolOu",
      {
        name: `${props.namespace}_SandboxAccountPool`,
    ...
    const allowedServicesScp = getSandboxAwsNukeSupportedServicesScp({
      namespace: props.namespace,
    });

    new CfnPolicy(scope, "SandboxAwsNukeSupportedServicesScp", {
      name: "SandboxAwsNukeSupportedServicesScp",
      description:
    ...
    const restrictionScp = getSandboxRestrictionsScp({
      namespace: props.namespace,
    });

    new CfnPolicy(scope, "SandboxRestrictionsScp", {
      name: "SandboxRestrictionsScp",
      description:
    ...
    const protectionScp = getSandboxProtectScp({
      namespace: props.namespace,
    });

    new CfnPolicy(scope, "SandboxProtectISBScp", {
      name: "SandboxProtectISBResourcesScp",
      description:
    ...
    const limitRegionsScp = getSandboxLimitRegionsScp({
      namespace: props.namespace,
      isbManagedRegions: props.isbManagedRegions,
    });

    new CfnPolicy(scope, "SandboxLimitRegionsScp", {
      name: "SandboxLimitRegionsScp",
      description:
    ...
    const writeProtectionScp = getSandboxWriteProtectionScp({
      namespace: props.namespace,
    });

    new CfnPolicy(scope, "SandboxWriteProtectionScp", {
      name: "SandboxWriteProtectionScp",
      description:
